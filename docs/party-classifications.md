...
3. Verify the way every revision here was verified: seed a container with the *previous* `seed.sql`,
   ...
   **`-1` is not optional.** `seed.sql` builds its `seed_*` staging tables as `TEMP ... ON COMMIT
   ...
   statement after the first fails with `relation "seed_leagues" does not exist`. Added 2026-09-19
   (revision 99), after the documented command failed exactly that way — **a verification snippet
   that cannot run is worse than none, because its output looks like a broken seed file.**
...
already cast reference these rows" — was true before 2026-08-12 and is NOT true now, and it was
corrected on 2026-09-16 (revision 95) when it turned out to be the thing making a routine rename look
dangerous.** Identity is `seed_key`, so a rename is an in-place update of the `name_*` columns on a
row whose `id` never changes; nothing is orphaned. What a rename actually costs is set out under נעם
in Conventions: the legacy `name` column is left stale, and the row joins the list that
`generate-tables.py` would silently re-key. **Still check `admin_edited` and the vote count first** —
not because a rename destroys votes, but because an admin-edited name will not be overwritten and you
should know that before assuming the seed applied.
...
- **`two-faction-list` stays refused, and now has a NUMBER instead of an inference.** The filing
  ...
  slots, against ביחד's 62/58. **A refusal that survives its first real measurement is worth more
...
- **`anti-conscription-exemption` added** — the sharpest of the three and it names the instrument:
  *"בלי קומבינות, בלי שטיקים ובלי חוקי השתמטות"*, and **נבטל את חוק מעצרי עריקים**. The row held
...
- **נעם** (`noam.png`, dark theme only since 2026-09-16 — see below) — **the only one of the four that is here because its source disappeared**,
...
- **הדמוקרטים** (`the-democrats-2026.png` light, `the-democrats-2026-dark.png` dark) — *(2026-09-16,
  ...
  every party path, so nothing else changed). Light: since revision 98c, that same transparent file
  cropped to its alpha bounds (800×343), unchanged — navy ink is what it was drawn for, and the
  light file and dark file are now exact colour swaps of each other. (Revision 98a had used the
  owner's fbcdn JPEG on its sky-blue tile, 339×156, replaced the same day on the owner's call.)
  Dark: the party's own **transparent** PNG
...
**2026-09-16 (revision 98b): `noam.png` is now the DARK-theme file only.** The repo owner supplied the
party's own light version (`noamlisrael.org.il`, the site header's `logo.png`, 690×222): the same
lockup in navy and cyan on white, cropped to 678×210 and self-hosted as `noam-light.png`, which is
what `logo_url` now names. The tile is registered for dark mode in `DARK_VARIANT_LOGOS` (keyed
`Noam for Israel`), so neither file reaches the recolour. The paragraph below is the pre-2026-09-16
reasoning, still true of the tile itself.
...
- ~~**`periphery-development`, `annexationist`/`sovereignty-annexation`, and
  ...
  **all three RESOLVED by the vocabulary sweep, 2026-08-11 (revision 19). See "The vocabulary
  ...
  extended.**
- **נעם → `נעם לישראל`: RENAMED 2026-09-16 (revision 95), on the repo owner's instruction.** The row
  is now seeded as `נעם לישראל` / `Noam for Israel` / `Ноам ле-Исраэль`; **`seed_key` stays `noam`**,
  which is what makes the rename an in-place column update rather than a new row.
  *(History, kept because the reasoning was right and only its premise expired: the row was held under
  the plain `נעם` from 2026-07 on the stated blocker that **the ballot name is not certified**,
  restated on 2026-08-11 against the relaunch on `noamlisrael.org.il`, the 2026-07-29 campaign opening
  under "חופשי להיות יהודי" and three named candidates — close to the bar בית ציוני - המילואימניקים
  cleared, but that one followed a **completed merger**, a structural fact, and this was a rebrand.
  Revision 68 then read the registrar and recorded **ballot name `נעם לישראל`**, and revision 93
  flagged the trigger as discharged.)*
  **What the old warning got wrong, and it is why this was gated at all.** Both that bullet and the
  "Renaming a party orphans its votes" section said a rename orphans votes. **It does not, and has
  not since the 2026-08-12 `seed_key` model** — `seed.sql`'s `UPDATE ... WHERE t.seed_key = s.seed_key`
  finds the row by key and rewrites `name_he`/`name_en`/`name_ru` in place, so `id` never changes and
  every `vote_upcoming_parties` and `party_lineage` row keeps pointing at it. The name-keyed adoption
  statement above it fires only for `seed_key IS NULL`, and the duplicate-insert `RAISE EXCEPTION`
  guard fires only when a seed row's key is *absent*. **Verified against production before editing**:
  `admin_edited` was `[]` (so all three name columns were writable), **0 votes**, 1 lineage link
  unaffected.
  **Two real consequences that the vote warning was obscuring**, and they are the things to check on
  the next rename:
  - **The legacy `name` column is NOT updated.** It is written only by the `INSERT`, so this row's
    `name` stays `נעם`. Harmless — nothing reads it — but a rename does not make the row internally
    consistent.
  - **`generate-tables.py` now rewrites this row's `seed_key`.** It derives the key from `name_en`
    via `slug()`, and `slug("Noam for Israel")` is `noam-for-israel`, not `noam`. **`noam` joins
    `joint-list` and `the-reservists` in the never-paste-a-regenerated-block list** — regenerating and
    pasting wholesale would change identity, fail the adoption match, hit `RAISE EXCEPTION` inside
    `init_db` and CrashLoopBackOff every backend pod. This is true of **any** rename whose new
    `name_en` does not slug back to the existing key, which is most of them.
...
- **`state-commission-of-inquiry` has ONE holder and SIX refusals, and its holder has withdrawn from
  ...
  **SWEPT AND RESOLVED — 2026-09-19, revision 99. The answer is NEITHER of the two options above:
  populate it, no; retire it, also no; document the distribution and leave `seed.sql` alone.**
  The sweep was prompted by the repo owner asking whether הדמוקרטים wants one too — it does,
  platform commitment #5, *"נקים ועדת חקירה ממלכתית ונשקם את מערכת הביטחון"* — and by the instruction
  to check the other rows, which is what produced the answer. **Nine refusals now, two of them added
  by this very pass after it had drafted the tag in.**

  | row | position | evidence |
  |---|---|---|
  | ישר | **has drafted the statute** — and *"נעשה את זה בשבועות הראשונים"* | `מתווה ישר לוועדת חקירה ממלכתית` PDF; ynet 17.09.26 |
  ...
  - **Retirement was drafted and then withdrawn, because the `periphery-development` precedent does
    not transfer.** That tag was retired because **none of its three holders had a line of
    justification** — the defect was on the rows. Here the holder is correctly tagged on its own
    evidence and the defect is the **absence everywhere else**. Removing it would assert that
    אל הדגל did not want a commission, which is false, and would edit a **withdrawn party's frozen
    record** to fix a problem living on the other seventeen rows. `seed.sql` is therefore untouched
    for this tag, on the repo owner's call and for the reason he gave.
  - **Populating it was refused because the distribution is FOUR-VALUED and the tag is boolean** —
    drafted the statute / wants one / wants an inquiry but rejects the statutory form / opposed. A
    single bit flattens all four.
  - **And it would cut at the wrong seam.** המילואימניקים והכלכלית and עמך ישראל sit in **opposing
    blocs** and hold the **same** position, and the tag's *name* — which encodes the statutory
    ממלכתית form — is what excludes them both. **This is the `two-faction-list` defect exactly: a tag
    whose name rather than whose evidence decides its membership**, and the two are now the same
    open question rather than two.
  - **The remaining membership is the opposition bloc**, which `bloc` already reconstructs, so the
    tag fails test (a) of the vocabulary sweep on top of everything else.
  - **What survives is the prose**, which is where line 783 said this distinction belonged all along:
    אל הדגל wants a commission, ישר has drafted the statute, and two rows across the blocs want an
    inquiry that is deliberately not a *ממלכתית* one.
  - **MEASURED, 2026-09-20 (revision 100), and it confirms the resolution from outside the party
    system.** An INSS poll this month finds **75% support establishing a state commission of
    inquiry — including 52% of COALITION voters** ([הארץ](https://www.haaretz.co.il/news/politics/2026-09-20/ty-article/.highlight/000001a0-bafc-d160-a3ae-beff516e0000)). ד"ר עידית שפרן־גיטלמן
    notes it is the one question in the survey where respondents' politics do **not** predict their
    answer, while every other war-related question splits on bloc. **A position held by
    three-quarters of the public and by a majority of the governing coalition's own voters is the
    definition of a tag that cannot discriminate** — the sweep reached that conclusion from nine
    party positions, and the polling reaches it from the electorate. Recorded because the sweep's
    verdict was an argument and this is a measurement of the same thing.
- **No tag exists for REFUSING a rotation or an alternating premiership — SIXTH item in the sweep
  queue, and both poles arrived in one week (2026-09-19, revision 99).** `unity-government` has two
  holders (כחול לבן, ישראל תחילה) and no negation. On 17.09 איזנקוט refuses it on principle —
  *"רוטציה לא עומדת על הפרק. אני מתנגד באופן עקרוני לכל הפטנט הזה של ראש ממשלה חלופי"* — while גנץ
  in the same week treats a rotation as a live option he might have to adjudicate
...
- **Two exclusion tags with different criteria and overlapping membership (2026-09-19, revision 99).**
  ...
  ישראל ביתנו) excludes the haredi parties too, who meet that criterion. Both are real and they are
  not the same tag — but nothing on the page says so, and a later pass will reasonably try to merge
  them. **Resolve alongside `two-state`/`pro-two-state`**, which is the same kind of defect.
...
| 2026-09-14 | revision 70 — **two new parties added to `upcoming_parties`: הציבור החרדי (`unaligned` · −2 / NULL / +1 · haredi) and ישראל תחילה (`unaligned` · +2 / +2 / −2 · secular), both scored from a complete published platform.** הציבור החרדי's `security` NULL is a **measured** absence — a keyword sweep over all eight platform chapters plus the אודות, fact-check and four campaign pages returns zero hits for עזה, פלסטינ, ריבונות, חמאס, איראן, מלחמה, נתניהו, קואליציה, גוש and ציונ — and the same sweep is why its `bloc` carries `hard-to-classify-bloc`: there is no coalition statement of any kind, and reading `bibi` off `sector: haredi` would be the inference-from-category this page refused for the Arab parties. It is the **first holder of the religiosity +1 band**, whose meaning was extended rather than the row rounded to a neighbour. ישראל תחילה is the opposite case on `bloc`: `unaligned` is its product (*"נשב עם כל מפלגה ציונית"*, *"לא ניתן יד לממשלה הנשענת על כוחות אנטי-ציונים. נקודה."*), so `hard-to-classify-bloc` is deliberately absent. Both rows' +3 bands were tested and refused with the test written down. Nine new tags, each on a verbatim first-party clause; `excludes-haredi-and-arab-parties` and `mizrahi-representation` were both considered for reuse and **refused** as half-claims. `seed.sql` +2 rows; `services/frontend/logos/israel-first.png` self-hosted, cropped to drop the *יש לך בית* tagline. |
| 2026-09-15 | revision 71 — **הציונות הדתית launched a judicial-system plan (סמוטריץ' + רוטמן, 15:30 IDT); the plan document is not published on any first-party channel. No axis moved, no tag added, `seed.sql` unchanged.** The party's own text is a five-item list already covered by `judicial-overhaul`; `pm-immunity-protections` and `governance-reform` are both reachable by the plan **as reported in the press** and both refused on that basis, with the same trigger — the party publishing the plan. Records the enumeration method (`page-sitemap.xml` answers "does the page exist" without guessing its slug; a 404 sweep cannot) and that certificate transparency is a floor, not a total. Opens a כחול לבן thread: the second post challenges גנץ to publish a legal plan, and that row holds no judicial or governance tag |
| 2026-09-15 | revision 72 — **הציבור החרדי `bloc` `unaligned` → `bibi`, `hard-to-classify-bloc` REMOVED (2 holders → 1, כחול לבן alone). Read against TWO broadcast interviews, both first-party, both published BEFORE the pass that created the row.** Sources: **ערוץ 7** studio interview with איציק ברנדויין, **31.08.26**, and **כיכר השבת** 24-minute studio interview with ישי כהן, **02.09.26**; both transcribed from the page, neither via a summary. כיכר settles the recommendation — asked *אתה אומר "אני הולך רק עם נתניהו"?* he answers **"חד-משמעית!"**, and asked whether he would be **אייזנקוט's** 61st seat, *"מה פתאום!"* — while **ערוץ 7, two days earlier, is what stops it being over-read**: asked twice whether he would join a right-wing government NOT headed by Netanyahu, *לייטנר לא התחייב לאדם מסוים* and *לא נקב בשם של מועמד*, naming the camp and the government's guidelines as the test (*"אנחנו נאמנים לגוש האמוני"*). Coherent, not contradictory: **the loyalty is to the bloc and within the bloc the candidate is Netanyahu**, which is what this column records — זהות's 2026-07-27 correction is the precedent from the other direction. Same shape as revision 41: the hedge was carried on one stated reason, *"there is no coalition statement of any kind"*, and there were two. **The durable finding is that revision 70's sweep was COMPLETE and its conclusion was still wrong, because the corpus boundary was the party's WEBSITE** — all 66 `sitemap.xml` URLs, zero hits for נתניהו/קואליציה/גוש, still true of the site, while the chairman had said it on camera **fourteen and twelve days earlier**. A site enumeration bounds the site, not what the party has said; the empty result read as a correct negative and no re-run would ever have surfaced it (the root `CLAUDE.md`'s "a pattern that can never match"). **New rule in `services/backend/CLAUDE.md`: search a new party's NAME and its chairman's in the press alongside enumerating its site, before writing down a measured absence** — a new party's public record may live entirely in interviews because it has not had time to write anything down. **`security` NULL HELD and is now one of the better-evidenced NULLs on the page** — three independent corpora, and the apparent challenge is named and rejected: *"אני בתפיסתי האישית ימין - ימין כלכלי, לאומי, ביטחוני"* is a self-placement on the map, which `bloc` takes and this axis cannot, since neither interview has a sentence on Gaza, sovereignty, יהודה ושומרון, a Palestinian state, Iran or borders (7 October appears as a manpower argument, not a doctrine). **economic −2 HELD, its caveat upgraded from aspiration to named instruments, and `claims-economically-liberal` REFUSED** — לייטנר is ימין כלכלי, backs סמוטריץ's milk reform and would abolish the הסתדרות as *"אנכרוניסטי"*, but the tag's founding case is a rhetoric-versus-**record** gap read against a sitting finance ministry and this party has no record at all; granting it on a platform/chairman gap would dilute it the way `service-conditioned-citizenship` was diluted. Trigger: an economic chapter carrying those instruments. **The `conscription-by-incentive` refusal survives on BOTH limbs, after a near-miss that is the pass's real lesson.** כיכר's *"מסיר את כל הסנקציות"* refutes the sanctions limb read alone — and it is the closing clause of the **learner** half; ערוץ 7 supplies the other half, *"מי שימלא את אחריותו יקבל תמריצים ומי שלא יעשה זאת ייתקל בתמריצים הפוכים"*, plus a ועד רבנים to fix *"איך עובר הגבול"* and *"מנגנוני פיקוח"*. This entry was drafted with that limb written off and only the earlier interview put it back: **a quote that refutes a stated reason must be read in its own paragraph before it overturns one.** **No tag added; four refused with the test written down** — `sanctions-on-non-servers` (*"תמריצים הפוכים"* names a direction, not an instrument; revision 41's standard, mirrored), an inverse sanctions tag (`scholar-exemption-retained` already **is** that claim), a women's-exclusion tag (would imply ש"ס and יהדות התורה do otherwise; and the two interviews disagree with each other — ערוץ 7 has **"אני בעד נציגות נשית חרדית"** against כיכר's dismissal) and `rabbinate-as-fourth-branch` (a party's own rabbinical council is not the state Rabbinate). **`core-curriculum` KEPT and qualified**: the interview asks for English and maths in *supplementary* frameworks for children who miss them at school, plus easier licensing of secular-teaching haredi institutions — opt-in supplementation, not a funding condition, so a reader scoring this row from the interviews alone would not reach the tag. `rabbinic-authority-led` corroborated and logged **as corroboration**, with לייטנר's own limit on it (*"במקום 150 רבנים… רשימה מצומצמת יותר"*) set against the אודות page's flatter claim. Also recorded: the party's first choice was absorption into ש"ס or יהדות התורה and it was rebuffed (the entry's revolt-against-the-parties reading, now first-party), and **it is an open merger candidate** — וינטר, אביר קארה *"רעיון מעניין"*, talks *"עם כל הגורמים בספקטרום הימני"*, run alone only if the threshold looks reachable alone. Trigger: the CEC filing; a joint list moves `on_ballot` and `party_lineage`, not an axis. `seed.sql` one row edited (`bloc`, one tag dropped, 14 tags → 13); no axis moved |
...
| 2026-09-16 | revision 82 — **הליכוד petitions the CEC chair against the voter-flight projects (הארץ, paywalled; body supplied by the repo owner). No axis moved, no tag added, `seed.sql` unchanged — here or on הדמוקרטים, a named respondent.** Filed against AID Coalition's `FLY&AID`/`Fly & Vote`, עמותת "המחנה הישראלי" and הדמוקרטים, claiming election bribery and a party-financing breach; the organisation says it supplies bureaucratic and logistical help only, every cost including the fare borne by the voter, no party condition. **The petition is first-party and is read as such; the opponent's gloss is not** — הליכוד disclaims the obvious reading (*"אינה מבקשת למנוע הצבעה... אלא מבקשת להבחין בין פעילות אזרחית כללית לבין פעילות בחירות מאורגנת, ממומנת ומכוונת"*), and recording AID Coalition's *"מבקשים למנוע מישראלים להצביע"* as this row's position would be the move revisions 44, 80 and 81 refused three days running. **A genuine vocabulary gap filed as a LEAD, not a sweep**: nothing in the 126 entries covers election administration or franchise access — not absentee voting (which Israel lacks, hence flights), not the voter register, not turnout-drive restrictions; `service-conditioned-citizenship` is about conditioning the franchise on service, a different question. One party on one filing is one data point, the same discipline that kept revision 80's press-freedom gap a lead. **Trigger**: any row publishing on expatriate voting, absentee ballots or the register. **הדמוקרטים: nothing recorded, and the refusal is the point** — a rival's allegation in a filing, with no response quoted, is the tier revision 44 wrote its rule for; written down so a later pass does not score it. Organisation figures (50–70k target, 30,000+ registrations, $75 deposits) are context, not evidence |
| 2026-09-16 | revision 83 — **הציונות הדתית: the party's AI campaign video dragging יצחק עמית, דפנה ברק־ארז and גלי בהרב־מיארה along the road (כאן, הארץ TV review). First-party material of the strongest tier and NOTHING moves.** `judicial-overhaul` already held; revision 30's line applies — this page tags what a party advocates, not how it says it, and the video is the existing position at maximum volume. Recorded for its **tier**: the party's own published campaign material, rarer on this row than it looks, which still has nothing published for 2026. Smotrich's expected defence (the video depicts clearing a **road blockage**; the real scandal is those who blocked the שב"כ and מוסד chiefs) is the same court-and-gatekeeper hostility `judicial-overhaul` carries, in revision 77's shape. **The critical minister is UNNAMED and therefore unattributable** — *"שר בכיר בממשלה"* on *"גרירות מבזות"*; an intra-coalition rebuke would be a real finding if it could be filed to a row, so it is left unrecorded rather than guessed at, and הרצוג belongs to no row. **מיקי זוהר's citizenship-stripping statement about the "נז"א" filmmakers is recorded against הליכוד (#13) and NOT tagged**: it arrives only via a hostile column's opening clause, revoking citizenship is outside his portfolio so it is stated intent rather than revision 77's ministerial record, and revision 49's candidate-advocacy rule stands. **Explicitly do not fuse it with revision 82's franchise lead** — same row, same day, adjacent-feeling, genuinely different questions, and merging them would manufacture a two-instance case out of one. **The column's analogy is REFUSED as evidence**: extending `voluntary-palestinian-emigration-incentives`, a tag earned from a drafted bill, to Jewish citizens by metaphor supplies no instrument. **Retrieval correction — `kan.org.il` is READABLE and revision 77 said it was not**: a browser-shaped `curl` (UA + `Accept`/`Accept-Language`/`Referer`/`--compressed`) returns **200** with the full body while WebFetch still 403s. **Fourth instance** of the rule after `timesofisrael.com`, `kachollavan.org.il` and `davar1.co.il`: a fetcher's 403 is a fact about the fetcher, not the site. Revision 77's Kan gap is now closeable |
...
| 2026-09-16 | revision 92 — **הציונות הדתית: a profile of פייגלין and the merger (הארץ magazine, 2026-09-14; body supplied by the repo owner). Almost everything was already decided, and the one thing that is not challenges a provenance claim this page makes in TWO places. No axis moved, no tag added, `seed.sql` unchanged.** **`population-transfer` may be NATIVE to this row rather than carried in**: both the אל הדגל entry and its 2026-08-10 planks note say it is *"זהות's tag, carried in by the 2026-09-01 merge"*, but the article supplies פייגלין's compulsion (*"כל תינוק בעזה הוא אויב"*, expulsion *"עד התינוק האחרון"*, *"בלי כיבוש, גירוש והתיישבות זה לא שווה כלום"*) **and says אורית סטרוק voiced the same position in the Knesset two months EARLIER**, from this row's own side, before the merger existed; its closing line has the top four — סמוטריץ', סטרוק, רוטמן, סוכות — as *"הקוראים לגירוש והתיישבות בעזה"*. **Challenged, not overturned**: הארץ *characterises* סטרוק's remark without quoting it, and a journalist's assertion about a first-party statement is not the statement. **The check: find סטרוק's Knesset remark, around March 2025.** **Not cosmetic** — the זהות subsection anticipates an un-merge and says a restored row *"starts with the tag"*, so an imported-tag record would silently strip `population-transfer` off this row on a split, which is a scenario the page has already written down as plausible. **Everything else corroborates**: the *"בלוק טכני"* framing with separate campaigns and coalition negotiations (`two-faction-list`), פייגלין at **#2** under זהות's registration, `cannabis-legalization` and the libertarian past with revision 68's refusal of זהות's `economic +3` standing (one of two motives in a technical bloc, revision 58's rule), `hardline-on-gaza` (siege, blocking aid, the 2014 *"כל סיכון נוסף... בלתי מוסרי"*, the 2019 אילון billboard) and `territorial-control-gaza`. **A candidate this page has NEVER mentioned: רעות חיים at #9**, founder of **צו 9** which blocked aid trucks into Gaza, **under international sanctions** — zero prior occurrences; recorded in prose on revision 57's line (the conduct is the row's own subject matter) and not tagged, since `hardline-on-gaza` covers the position and a candidate's organisation is not a party instrument. **`bloc` stays `bibi` — the THIRD within-bloc attack today that does not move it**: *"כל ראש ממשלה... צריך להניח את המפתחות, ובוודאי גם נתניהו"* immediately followed by *"אני לעולם לא אקים כאן ממשלת שמאל"*, the same shape as עוצמה יהודית's and וינטר's quarrels in revision 88 — **a quarrel inside a bloc is not an exit from it**, three rows, one day. **Recorded and not tagged**: the defence-portfolio demand, נתניהו's reported pressure for the merger with both parties declining to say what was promised, the שב"כ conspiracy claims about Rabin and 7 October (a sixth frame-not-instrument refusal of `populist`'s kind), the **sedition conviction** over anti-Oslo road-blocking (a record, not a position, but inside `settler-movement`/`anti-oslo`), and his downplayed opposition to LGBT rights, too weak to put `anti-lgbt` on a row that has never held it |
| 2026-09-16 | revision 93 — **נעם: four sources, one of them the chairman's own by-lined op-ed (כיפה 2026-09-09, כיכר השבת 2026-08-13, ערוץ 7, ישראל היום). No axis moved, no tag added, `seed.sql` unchanged — and one ACTIONABLE item is flagged rather than executed.** Mixed-date corpus, stated because revision 30's date-bounded-sample finding is about exactly this. **⚠ The `נעם` → `נעם לישראל` rename trigger looks DISCHARGED and nobody executed it**: revision 18 held it pending *"ballot certification"*, revision 68 read **ballot name `נעם לישראל`** at the registrar, all four sources use it. **Flagged for the repo owner, not applied** — filing and CEC certification may not be the same step and the trigger names the second; this document carries a section on **renaming a party orphaning its votes**; `seed_key` stays `noam` either way. A scope-and-irreversibility call, not a research one. **`rabbinic-authority-led` gets its purest evidence on this page**: הרב צבי ישראל טאו's letter settles whether the party *runs at all* — *"עמדתנו הנאמנה היא ש'נעם לישראל' תתמודד עד להצבעה בפועל בקלפי"* — and the party publishes it as the answer; every prior citation was doctrinal. **`on_ballot` TRUE on TWO independent first-party statements** against real withdrawal pressure; הרב יוסף קלנר's *"ברגע האחרון אבי מעוז יודיע שהוא פורש"* and *"במקרה הטוב יש לנעם 40 אלף קולות"* are an outside rabbi's estimate, not scored. **A hostile witness confirming the POSITIONS while disputing the PROSPECTS** — קלנר calls the anti-progressive/anti-LGBT mission *"רעיון טוב"* and won't vote for it (*"אין סומכים על הנס"*), so `anti-lgbt` and `anti-progressive` are corroborated by someone with every incentive to deny them; **what a party stands for and whether it will cross the threshold are different questions.** **`judicial-overhaul`/`anti-judicial-review` reach their sharpest instrument form**: פסקת ההתגברות passed **BEFORE the swearing-in**, *"חלק מהתנאים המקדימים להקמתה ולא הבטחה שתידחה"*, plus splitting the AG's role and decisive elected weight in appointments — revision 18's third-instance vocabulary split is now the better-evidenced side of itself. **A pre-swearing-in coalition precondition is the THIRD instance today, third row, third bloc** (עמך ישראל/conscription revision 41, כחול לבן/service revision 86, this row/override) — **three subjects, one instrument, and it is not a position but a way of HOLDING one**; a sharper founding case than several queued items had, **filed as a sweep-queue candidate, not minted**. מעוז's *"במהות היא כשלה"* spine is all already tagged (education, deep state, courts driving *"מדינת כל אזרחיה"*). **Two statements recorded and NOT tagged**: *"כמות המורות והגננות הערביות בחינוך היהודי הולכת וגדלה"* — **`jewish-supremacist` refused**, since that tag's standard here is עוצמה יהודית's and an objection to Arab staffing framed as identity preservation is a different kind of claim, not a milder version; and the IDF ethical code as written by *"תועמלני שמאל קיצוניים"* against *"ערך הניצחון מעל כל ערך אחר"* — **no tag covers military-ethics doctrine**, one holder, noted. **A polar asymmetry named but NOT filed**: `state-of-all-its-citizens` has no opposite pole, but unlike the gun-control gap it is probably already covered by `halakhic-state`/`hardal`/`sovereignty-annexation` — that gap was filed because *nothing* covered the other pole, which is not true here |
| 2026-09-16 | revision 94 — **cross-row: an analysis column on land enforcement and the Arab vote (הארץ 2026-09-14; body supplied by the repo owner). Nothing moves, and the useful half is that it contains NO first-party text at all.** **A source type this page has not yet named: an argument about parties with nothing any party said.** Every refusal this week had the shape *"the party said X, the rival said Y"* — a rival characterising (44, 82, 87), a friendly outlet transmitting (81, 89), disputing witnesses (85), a hostile reviewer quoting (90). **Here there is no X**: the column names סמוטריץ', סטרוק, בן גביר and צבי סוכות and quotes none of them, and says of itself *"אין הוכחה לתוכנית מסודרת"*. The danger is specific — a well-argued column about the right subjects reads like a finding, and the temptation is to promote its characterisations because they are *about* parties rather than *by* them. **The Kaminitz / land-enforcement gap gets a THIRD sighting, and this one supplies the substance rather than the name**: demolition orders, eviction demands and fines of hundreds of thousands of shekels against decades of state delay in approving master plans and expanding building areas. Revision 44 queued it behind the two Arab rows, revision 85 saw it from a **non-Arab** row (the Bedouin heads putting חוק קמיניץ to ישר, some framed as amendment not repeal), this is the third. **Upgraded from queued debt to a named sweep candidate**, on revision 93's reasoning — three independent sightings across blocs is a founding case and the remaining work is one membership pass, not more evidence. **A one-holder dimension noted and NOT filed: Judaization of the Negev and Galilee.** The column calls it coalition-wide; this page has it as first-party text on **exactly one row** — ישראל תחילה's אדמה chapter (*"יצירת רצפים התיישבותיים"*, *"ביטול תוכניות היוצרות רצפים טריטוריאליים ערבים"*, *"קרקע שלא אנחנו מחזיקים בה – יחזיקו בה הערבים"*). `pro-settlement` is about the West Bank and does not reach it, and `periphery-development` was retired; one holder is not a dimension, and the אדמה chapter is the text a second row would be compared against. **Recorded as background, not evidence**: the thesis that שומר החומות and the 2021 mixed-city riots, not 7 October, turned Jewish opinion is about the electorate, and the *"לא להתעמת – להצביע"* prediction is a forecast — **both relevant to nothing on this page**, stated plainly so the article does not look partially used |
...
| 2026-09-16 | revision 98b — **נעם לישראל gets a light-theme logo; no axis moved, no tag added.** The party's own site header logo (navy/cyan on white) is self-hosted as `/logos/noam-light.png` and becomes `logo_url`; the existing navy tile `noam.png` moves to dark mode only via `DARK_VARIANT_LOGOS` (`Noam for Israel`), the second party there after הדמוקרטים. Verified by rendering the real `logos.js`/`style.css` in headless Chromium in both themes; `test-logo-assets.sh` green |
| 2026-09-16 | revision 98c — **הדמוקרטים light-theme logo replaced, on the repo owner's call; no axis moved, no tag added.** `/logos/the-democrats-2026.png` is now the party's own transparent PNG (`victory26.democrats.org.il`) cropped to its alpha bounds, replacing revision 98a's sky-blue fbcdn tile. Same URL, so `seed.sql` is unchanged; the dark file is untouched and is now the exact navy↔white swap of the light one. Verified in headless Chromium in both themes |
| 2026-09-16 | revision 98d — **הרשימה המשותפת gets a light-theme logo; no axis moved, no tag added.** The repo owner's fbcdn JPEG (teal wordmark and the four-colour stripes on white, 1702×630) is cropped to the wordmark with the existing tile's proportional margins and self-hosted as `/logos/joint-list-2026-light.png`, now `logo_url`. The teal tile `joint-list-2026.png` moves to dark mode only via `DARK_VARIANT_LOGOS` (`The Joint List`), the third party there. Verified in headless Chromium in both themes; `test-logo-assets.sh` green |
| 2026-09-19 | revision 99 — **five sources in one pass: the CERTIFIED הליכוד list, ביחד's religion-and-state plan, הדמוקרטים's conscription paper, the ynet איזנקוט interview and the בג"ץ אלקטור ruling. Six tags added across four rows; no axis moved; nine refusals recorded.** **הליכוד's "Realized list" was the PRIMARY RESULT, not the filed list** — certified order differs from slot 3 down, **slot 11 is ברדוגו** (not `אורן דוברונסקי (reserved, unconfirmed)`), and **טלי גוטליב is on no slot of the 120** though this page placed her at 20. `two-faction-list` **stays refused and gains a number**: 117/120 to הליכוד against **3** to תקווה חדשה (#9, #88, #89), where the prior refusal reasoned from the CEC page title; ביחד's comparable filing is 62/58. **ביחד +`civil-marriage` (21 → 22) and a sentence in "The axes" corrected against it** — that band asserted ביחד had no civil-marriage position while its plan commits to *"ברית זוגיות אזרחית במשרד הפנים"* and to *"הרבנות הראשית לא מבוטלת. הבלעדיות שלה כן"*; **religiosity HELD at −2** because the plan keeps state religious funding and keeps conversion orthodox, which is the funding criterion that same sentence already named for ישר and בית ציוני — **same verdict, correct reason, third attempt**. **הדמוקרטים +3 (23 → 26)**, corpus 15 → 16, revision 62's edition probe run first and clean (`(1)`/`(2)` both 403): `anti-conscription-exemption` (*"בלי חוקי השתמטות"*, **נבטל את חוק מעצרי עריקים**), `state-haredi-education` (*"מערכת חינוך ממלכתית־חרדית"*), `arab-civil-service`; **security −1 held for the THIRD time** on revisions 40/62's argument, now against a document devoted to the subject; `anti-annexation` gains a **military** leg (*"נפסיק לרתק כוחות להגנה על מאחזים מבודדים"*) beside revision 40's fiscal one. **`workforce-integration` was drafted in and then REFUSED** — its only support is the purpose clause of the school system already earning `state-haredi-education`, so it records one sentence twice (revision 62's `public-service-reform` ground), and the draft had additionally fused an **Arab-society** clause into a haredi tag. **ישראל ביתנו +`excludes-haredi-and-arab-parties` (15 → 16)** — the Open question said the tag was undocumented and could not move until someone wrote down what it means; *"ממשלה ציונית, ללא נתניהו, המפלגות הערביות והחרדיות"* supplies the definition and the second holder. **עמך ישראל +`hardline-on-gaza` (7 → 8)**, the thinnest row's first doctrinal statement (*"תפיסת ההכרעה"*, *"לדחוק את האוכלוסיה העזתית דרומה ולייצר מצור חכם"*); **`population-transfer` REFUSED** — the same words serve either claim and only the surrounding sentence decides. **`state-commission-of-inquiry` SWEPT and resolved after nine refusals, two added by this pass after it had drafted the tag in**: not populated (the distribution is four-valued and the tag is boolean), **not retired** (the `periphery-development` precedent does not transfer — that tag's holders were unjustified, this one's is, and אל הדגל is a withdrawn party's frozen record), `seed.sql` untouched. The seam the tag cuts at is wrong: **המילואימניקים והכלכלית and עמך ישראל hold the same position from opposing blocs** and the tag's *name* excludes them both — the `two-faction-list` defect. **Three new sweep-queue items**: no negation for `unity-government` (both poles dated within seven days — איזנקוט refusing rotation *"באופן עקרוני"*, גנץ entertaining it), `sectoral-budgeting` recording only the receiving end (הדמוקרטים against **₪20bn** of *"כספים קואליציוניים"*), and the two exclusion tags. **Refused and written down so a later pass does not re-score them**: the ברדוגו communications portfolio (two closed conversations, three unnamed sources — the revisions 44/80/81/85 tier, and the party denies the premise), ברדוגו's שב"כ meeting (a candidate is not a position), the שב"כ registration page (a state body's conduct, no party statement), the twelve mutual disqualification petitions (**the same instrument fired in both directions is campaign ritual**), עוצמה יהודית's Druze support letter (**first-party publication, third-party content** — and an automated rendering misattributed it to בן גביר when the letter is to **וסרלאוף**), the אלקטור workaround (machinery, not programme), and ישר's `anti-indicted-pm` (*"כל חברי הקבינט ב-7 באוקטובר"* is broader than the tag). **Fetching note**: `gov.il` sits behind a Cloudflare JS interstitial that returns **403** to WebFetch and to browser-shaped `curl` alike; the certified list was read with Playwright Chromium, the same class of finding as revision 43's mako/i24 note. **Verification note**: the revcheck snippet in `services/backend/CLAUDE.md` omits `psql -1`, and the seed's `ON COMMIT DROP` temp tables make every statement fail without it |
| 2026-09-20 | revision 100 — **three sources, NOTHING moves: no axis, no tag, no `seed.sql` change.** The third such pass on this page, and the first where every item was already settled in the opposite direction from the reading that prompted it. **הליכוד is the women's-representation survey's sharpest case and gets nothing** — מיזם 50/50 scores it *"חזרה 15 שנים אחורה… רק שתיים או שלוש נשים במקומות ריאליים"* against five lists it counts balanced or near-balanced (ישר, הדמוקרטים, כחול לבן at 50-50; ביחד and עמך ישראל close), and **the list-is-not-a-programme rule declines to record it in either direction**: the rule was set by refusing ישר the tag *"against a much stronger fact"* — its chairman's own commitment plus 14 of the top 30 — so **a third party's count cannot earn what a party's own statement could not**, and a rule that only ever cost opposition rows a tag would be worth distrusting. Same rule applied where it costs the other way: **הציבור החרדי places women at #3 and #4**, unprecedented for a haredi list (ש"ס and יהדות התורה carry none), and earns nothing. **`gender-equality` therefore also stays off עמך ישראל.** **סמוטריץ': *"בקדנציה הבאה נייהד את הגליל כמו המהפכה שעשינו ביהודה ושומרון. על זה הבחירות"*** — a **second** instance of the `/judaization/` programme read 2026-08-26 (**״כן. מייהדים.״**, 20 Galilee settlements, *"קרקע שלא אנחנו מחזיקים בה – יחזיקו בה הערבים"*), now first-person with a timetable and elevated from a programme page to the stated stake of the vote. **Still not minted**: revision 31 refused the demography-inside-the-Green-Line tag *on scope, not on evidence*, and **a second instance from the same row does not answer a scope argument** — minting here would be the audit-coverage failure the `state-commission-of-inquiry` sweep was about. Logged that `population-transfer`, already on that row, does **not** absorb it: that tag is about Palestinians in the territories, this is land policy inside the Green Line aimed at citizens. **ישר: a FOURTH coalition principle surfaces and DATES revision 85's evidence** — שאול מרידור on אופירה אסייג gives *"מפלגה חייבת להכיר בחמאס כארגון טרור"*, which the reviewer says was not among איזנקוט's three stated principles and *"צץ בעקבות הפרסום של מוחמד מג'אדלה"*, i.e. appeared during the controversy revision 85 was written about. **The set of principles is not fixed; it grew by one under pressure.** `excludes-anti-zionist-parties` **survives and the check mattered** — it was granted on the Jewish-and-democratic-state criterion, the FIRST of the three, so a reactive fourth neither supports nor undermines it; **had the tag rested on the Hamas clause, this would have required withdrawing it.** Dating recorded as **contested, not settled** (one columnist, no transcript), per revisions 81 and 96. **The `state-commission-of-inquiry` resolution is now MEASURED from outside the party system**: an INSS poll finds **75% support, including 52% of coalition voters**, and שפרן־גיטלמן notes it is the one war-related question where politics does not predict the answer. The sweep concluded the tag cannot discriminate from nine party positions; the polling reaches the same place from the electorate. **Refused and written down**: טל שניידר's characterisation of סמוטריץ' on women in the army (a journalist on a leader, revision 44's tier whoever speaks), ארז תדמור's *"מרד הגנרלים"* as נתניהו's campaign manifesto (a candidate's book is not a party position — though **#27 checks out against revision 99's certified filing**, the cheap cross-check revision 24 ran), the UAE's denial of the בן זאיד warning against איזנקוט's claim (neither is a party position), and the column's account of מנסור עבאס's motives (a commentator on a third row). **The coalition agreements ARE first-party and still add nothing**: the *"הגשמת הזהות היהודית במרחב הציבורי"* law and permitting service-provider discrimination on religious grounds are signed party text, and `halakhic-state` already names them — logged as the strongest form that tag's evidence has taken |
