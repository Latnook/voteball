...
**This repository is public, and it contains the AWS account id, the RDS endpoint hostname, the real
domain, and seven ARNs naming real IAM roles, an ACM certificate, a WAF ACL, an SNS topic and an S3
bucket.** None of that is an accident, and none of it is a credential. Recorded here so a reader can
check the reasoning instead of assuming an oversight.

**Where it is.** In `HEAD`, essentially nowhere any more — but that is recent, and it changes nothing
about the paragraph below, which is the part that matters.

Until **2026-09-08** `charts/voteball/values.yaml` carried seven occurrences and had to: ArgoCD
deployed what was committed, so those fields could not be placeholders without the first sync
reverting the cluster to a stale image tag and landing every pod in `ImagePullBackOff`. Nine
environment-identity fields then moved into the ArgoCD Application's `helm.parameters`, which
`scripts/render-argocd-app.sh` renders from `terraform output` and applies directly — template in git,
values never. `values.yaml` now ships `REPLACED-BY-ARGOCD` placeholders and `sync-values-from-tf.sh`
manages exactly one field, `image.tag`, whose history is the rollback mechanism and which leaks
nothing.

By volume the chart was always a rounding error: **`docs/eks/evidence/` held roughly 1,400
occurrences**, because raw `kubectl` and `terraform` output naturally contains ARNs and registry
hostnames and those captures were unedited on purpose. That directory — 110 files, 4.6 MB — was
**deleted on 2026-09-09**, along with the four `scripts/capture-*` scripts that produced it, once the
project stopped being submitted anywhere. The chaos drills survive and still write transcripts, now to
a gitignored `drill-output/` rather than into the repo.

**Deleting it un-published nothing**, which is the whole point of the next paragraph. It stops the
pile growing; it does not shrink it.

**Why it is not retractable, which is the part that matters.** The id spans **48 commits** going back
to the earliest EKS work. Removing it from `HEAD` would un-publish nothing: git history is permanent,
GitHub keeps unreferenced objects reachable, forks and caches persist, and public repositories are
scraped continuously. An AWS account id also cannot be changed. So the useful question is not "can we
take it back" (no) but "does knowing it grant anything" — and the answer is no, by construction:

| What an attacker learns | Why it is inert |
|---|---|
| `arn:aws:iam::…:role/voteball-worker-irsa`, `…/voteball-backup-irsa` | Both trust **only** the cluster's OIDC provider, conditioned on an exact `sub` (`system:serviceaccount:devops-app:worker`) **and** `aud`. Assuming either requires already being that ServiceAccount in that cluster. No `voteball` role trusts a wildcard or an external account. |
| `voteball-eks-db.….rds.amazonaws.com` | `PubliclyAccessible: false`, storage encrypted, in the isolated DB subnet group. AWS publishes RDS DNS regardless of reachability, so the name resolves — to a private address with no route from the internet. |
| `voteball-rollups-<account>` | All four public-access blocks are `true` on the bucket. |
| the ECR registry hostname | Pulls require `ecr:GetAuthorizationToken`; the repositories are private. |
| `voteball.latnook.com` | Intended to be public. It is a public poll. |

**An account id is an identifier, not a secret** — AWS treats it as non-sensitive, and every door it
points at here is conditioned on cluster identity rather than on knowing the id. The residual risk is
genuine but low: a more convincing phishing pretext, and shorter reconnaissance *for an attacker who
has already obtained a credential by some other route*. That second clause is the reason the account's
credential posture, not this repository's contents, is where the effort belongs.

**The dissenting view, and the check that answers it.** The strongest argument that account ids *are*
secrets ([Plerion, 2024](https://www.plerion.com/blog/the-final-answer-aws-account-ids-are-secrets))
does not claim the id itself grants anything — it claims the id is the key that makes *other* people's
mistakes findable: publicly shared EBS snapshots, AMIs, RDS snapshots and buckets are enumerable once
you know whose account to look in. That is a claim about the account, not about the repository, and it
is checkable rather than arguable. Measured **2026-09-09**, all read-only:

| Enumerable via the account id | Result |
|---|---|
| EBS snapshots shared with `all` or another account | 0 of 9 owned |
| Public AMIs | 0 of 3 owned |
| RDS snapshots with a `restore` attribute set | 0 of 52 manual (and none automated) |
| S3 buckets with a public policy, or without all four public-access blocks | 0 of 3 |
| ECR repositories with a wildcard principal, or any `ecr-public` repository | 0 of 7, and 0 public |

So the id is inert in the only way the dissent says it might not be. Re-run this check after any
change that shares a snapshot or an image — that, not the repository, is what would make it matter.

**What this means for a forker.** Nothing in `terraform/`, `scripts/`, `ci/` or `services/` hardcodes
the account, region or domain — identity lives in `terraform/voteball.tfvars` (gitignored) and
`terraform output`, read through `scripts/lib/config.sh`. Since 2026-09-08 `values.yaml` is no longer
even a partial exception: nine of its ten environment fields are supplied by the ArgoCD Application's
...
and its `403`s look exactly like an outage (the evidence-capture script throttled to 2 req/s for
this reason, until it was deleted with the rest of the submission tooling on 2026-09-09; the same
constraint applies to any probe you write next). And as a policy question, a large shared NAT — a university or corporate egress — could
...
`services/` had changed. Captured in the CI console log of 2026-08-17, which was also
the evidence that a failed scan **blocks the deploy**: `Push to ECR`, `Publish Metadata` and
`Trigger CD` all reported `skipped due to earlier failure(s)`, so nothing reached ECR and
`application-cd` never ran. (That transcript lived in `docs/eks/evidence/`, deleted 2026-09-09 — the
behaviour is still asserted by the pipeline itself, whose Trivy gate fails the build before the push
stage runs.)
