...
|---|---|---|---|
| `/metrics` | GET | none | Prometheus scrape target (multiprocess-aggregated across gunicorn's 2 workers). No public route: nginx proxies only `/api/*`, so this path is unreachable from outside the cluster; in-cluster reach is gated by NetworkPolicy, not by an auth check here |
| `/health` | GET | none | backend **startup** and **readiness** probe target — not liveness, which is a `tcpSocket` check on :5000 so that an RDS outage can never restart the pod. Returns a static `{"status": "ok"}` and touches no database, so a passing probe means "gunicorn is serving", not "RDS is reachable". Not reachable publicly: nginx proxies only `/api/*`, so `/health` 404s at the ALB — which is why `scripts/ci/smoke-test.sh` deliberately skips it |
| `/api/options` | GET | none | leagues/clubs/previous_parties/upcoming_parties (each with `name_en`/`name_he`/`name_ru`), consumed by both frontend pages; every party also carries `families` (array of strings, `[]` when none) and `family_evidence` (`'record'` \| `'platform'` \| `null`) — `upcoming_parties` are seeded with real values, `previous_parties` always return `[]`/`null` (families are computed from upcoming-election votes only, see `docs/design/2026-07-30-party-families-club-traits-design.md`); each league also carries `has_divisions` (boolean — whether *this league* renders division headers) and each club carries `group_label` (nullable string, `'A'`–`'D'` — *which* division, UEFA Nations League only, seed-only/not admin-editable) — see `docs/design/2026-08-07-nations-league-design.md` decision 1; each **upcoming** party also carries `on_ballot` (boolean) — `false` means the party withdrew, merged or split and is no longer standing. **This endpoint deliberately returns off-ballot parties anyway**: `admin.js` reads it too and is the only screen that can restore one, so the filtering lives in `vote.js` (which hides them) and in `/api/vote` (which rejects them) |
| `/api/vote` | POST | none, cookie-deduped | body `{"team_picks": [{"league_id", "club_id"}, ...], "previous_vote_status", "previous_party_id", "upcoming_vote_status", "upcoming_party_ids"}`; `team_picks` needs ≥1 entry and a `club_id: null` ("just this league") entry can't coexist with specific-club entries in the same league; there is **no per-`league_id` cap** — the cap is ≤3 clubs per **domestic** league, counted from each club's own `{league_id, domestic_league_id}` minus any league with `is_club_cup` (the three UEFA club cups), never from the `league_id` the pick was filed under, so a club with no seeded domestic league is uncapped; each `club_id`/`league_id` pair is validated against that club's real `{league_id, domestic_league_id}`; sets `voteball_token` cookie (1yr); 409 on repeat vote; 400 if `upcoming_vote_status=considering` with no `upcoming_party_ids`, or if `upcoming_party_ids` has more than 3 entries, or if any `upcoming_party_ids` entry names a party with `on_ballot = false` (error `one or more selected parties are no longer standing`, metric reason `party-not-on-ballot`) — client also validates all of this before submitting |
...
| `/api/results/clubs-breakdown` | GET | none | no params; per-club totals across all leagues; each entry is `{"club_id", "previous": [{"party_id", "count"}, ...], "upcoming": [{"party_id", "count"}, ...]}` — `previous` from `rollup_previous`, `upcoming` from `rollup_upcoming`, each deduped per club+party (backs the results-page Traits tab, `docs/design/2026-07-30-party-families-club-traits-design.md`) |
