...
A transport failure logs curl's exit code with a name (`curl exit 6: could not resolve host`,
`7: could not connect`, `28: timed out`, …). **Exit 6 gets its own time budget**, `SMOKE_DNS_WAIT`
(default 900s, shared by all three checks), and does not consume a retry. After a rebuild, a lookup
made before external-dns created the A record can be cached as "no address" for up to 15 minutes by
the VPC resolver (see `scripts/verify-public-dns.sh`), and a rollback changes the image, never DNS.
Failing sooner would only roll back a good deploy, which is what CD #1 did on 2026-09-15.
