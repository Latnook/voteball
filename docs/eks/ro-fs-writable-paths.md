...
| worker   | `/tmp` | The heartbeat file `/tmp/heartbeat` (see `heartbeat.py`, `DEFAULT_PATH`). Mount an `emptyDir` at `/tmp`. **All three** of the worker's probes read this file's mtime — startup, readiness and liveness (liveness alone until the startup probe was added 2026-08-06). |
...
- **Heartbeat path is overridable** via the `HEARTBEAT_FILE` env var, but defaults to `/tmp/heartbeat`;
  keep the chart's `emptyDir` mount, the env var (if set), and **all three** worker probe paths in
  agreement — startup, readiness and liveness each `stat` it independently, so changing the path means
  changing it in three places in `worker-deployment.yaml`, not one.
