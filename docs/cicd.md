...
The 153 tests in `services/{backend,worker}/tests/`, run against a **real** Postgres — both
...
| A commit you expected to build finishes `NOT_BUILT` immediately | The commit **message** contains the skip marker anywhere, including merely *writing about* it. The Guard fails safe toward skipping | Expected. Amend the commit message and push again |
