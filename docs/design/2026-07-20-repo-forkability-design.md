...
> **Criteria 3 and 6 were superseded the same day this landed (2026-07-20), and are left as written
> because this is a dated record.** Both assume `charts/voteball/values.yaml` can be free of the
> account id and domain. It cannot: ArgoCD deploys what is on `master`, not what is on an operator's
> disk, so those fields must be committed with **real** values or the first sync reverts the cluster to
> a stale image tag and every pod lands in `ImagePullBackOff` — observed on the 2026-07-20 rebuild.
> `values.yaml` is therefore the one deliberate exception, regenerated by
> `./scripts/sync-values-from-tf.sh` rather than hand-edited, and criterion 6's real scope is
> `terraform/`, `scripts/`, `ci/` and `services/` — which do remain clean. The exposure this creates,
> and the verified reasons it is inert, are written up under "What is deliberately public, and why that
> is safe" in [`../security.md`](../security.md). Flagged 2026-08-17 when the account id in the chart
> was queried; the criteria had contradicted the accepted design for four weeks.
