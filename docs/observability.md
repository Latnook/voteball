...
| Availability reads `1` after a rebuild, and the canary logs `curl: (6) Could not resolve host` | A NOERROR-with-no-A answer from before external-dns created the record is cached; the name already existed (a TXT record), so it was NODATA rather than NXDOMAIN. RFC 2308 caps that at `min(SOA TTL, SOA MINIMUM)` = **15 minutes** here, not the 86400 this row used to claim — and the copy that outlasts everything is the **VPC resolver's**, not CoreDNS's (30s) | `./scripts/verify-public-dns.sh` (deploy.sh step 11c runs it). The tell: **TXT resolves, A does not.** If it reports the answer is held upstream, wait it out — restarting things cannot clear that cache and visitors are unaffected |
