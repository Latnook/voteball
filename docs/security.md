...
**Where it is.** `charts/voteball/values.yaml` carries seven occurrences, and it *has to*: ArgoCD
deploys what is on `master`, not what is on an operator's disk, so those fields must be committed with
real values or the first sync reverts the cluster to a stale image tag and every pod lands in
`ImagePullBackOff`. See the forkability note at the top of `CLAUDE.md`. By volume, though, the chart is
a rounding error — **`docs/eks/evidence/` holds roughly 1,400 occurrences**, because raw `kubectl` and
`terraform` output naturally contains ARNs and registry hostnames, and those captures are unedited on
purpose.
...
`terraform output`, read through `scripts/lib/config.sh`. `values.yaml` is the single exception and is
regenerated by `./scripts/sync-values-from-tf.sh`, so a fork replaces all ten fields by running that
