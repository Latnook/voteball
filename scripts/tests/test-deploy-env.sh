...
# GRAFANA_GITHUB_TOKEN joined the other four on 2026-08-24 (step 3c, scripts/seed-grafana-secret.sh) -- it is
...
set -euo pipefail
. ./block.sh
env | grep -E '^(ADMIN_USERNAME|ADMIN_PASSWORD|JENKINS_ADMIN_USER|JENKINS_ADMIN_PASSWORD|GRAFANA_GITHUB_TOKEN)=' | sort || true
EOF
...
GRAFANA_GITHUB_TOKEN=filetoken
EOF
}
...
for expect in ADMIN_USERNAME=filename ADMIN_PASSWORD=filepass \
              ...
              GRAFANA_GITHUB_TOKEN=filetoken; do
...
echo "--- an explicit GRAFANA_GITHUB_TOKEN override beats the file too ---"
out="$(GRAFANA_GITHUB_TOKEN=envtoken probe)"
grep -qx 'GRAFANA_GITHUB_TOKEN=envtoken' <<<"$out" || fail "environment did not win over deploy.env for GRAFANA_GITHUB_TOKEN; got: $out"
