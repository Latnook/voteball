...
PYTHON_GROUP=(
  ...
  # already lived here rather than in GIT_GROUP because it keeps company with the other doc/config
  ...
  # Needs neither python3 nor git (bash + a fake `aws` on PATH, grep, awk, seq) -- confirmed passing
  # inside a bare python:3.12-slim on 2026-08-23, per the rule above. Never touches AWS.
  test-notify.sh
